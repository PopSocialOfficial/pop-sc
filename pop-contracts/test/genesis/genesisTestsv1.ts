...
describe("Genesis NFT testing", function () {
  ...
  beforeEach(async () => {
    ...
    const GenesisNFT = await ethers.getContractFactory("GenesisV1");
    genesisNFT = await upgrades.deployProxy(
      GenesisNFT,
      [100, 0, 0, fundRaiseClaimer.address],
      { initializer: "initialize" }
    );
    ...
    await genesisNFT.setWhitelisted(
      accessories.map((contract) => contract.contractAddr),
      accessories.length
    );
    await genesisNFT.setAccessoryOrder(
      accessories.map((contract) => contract.contractAddr)
    );
    ...
    await hatNFT.connect(relayer).mintBatch(bob.address, [1, 2], [1, 1], []);
    await clothesNFT
      .connect(relayer)
      .mintBatch(bob.address, [1, 2], [1, 1], []);
  ...
  it("Should have whitelisted contracts", async function () {
    for (let i = 0; i < contractsToWhitelist.length; i++) {
      expect(
        await genesisNFT.whitelistedContracts(contractsToWhitelist[i])
      ).to.equal(true);
    }
  });
  ...
  it("Should remove whitelisted contract", async function () {
    ...
    expect(await genesisNFT.whitelistedContracts(hatNFT.address)).to.equal(
      false
    );
  });
  ...
  it("Should not be able to claim if not whitelisted", async function () {
    ...
    ).to.be.revertedWith("Genesis: invalid merkle proof");
  });

  it("Should not revert if equip accessories on Genesis NFT in the right order", async function () {
    // prepare bob's wallet
    // await hatNFT.mintBatch(bob.address, [1, 2], [1, 1], []);
    // await clothesNFT.mintBatch(bob.address, [1, 2], [1, 1], []);
    ...
    await genesisNFT
      .connect(bob)
      .safeMint(bob.address, merklTreeRoot.getHexProof(keccak256(bob.address)));
  ...
  it("Should revert if not token owner and equip", async function () {
    ...
    const merklTreeRoot = generateMerkl([owner.address, alice.address]);
    await genesisNFT.setWhitelistMerkleRoot(merklTreeRoot.getHexRoot());
    await genesisNFT.safeMint(
      owner.address,
      merklTreeRoot.getHexProof(keccak256(owner.address))
    );
    ...
    await expect(
      genesisNFT
        .connect(bob)
        .equipAccessories(1, accessoriesToEquip, { gasLimit: 800000 })
    ).to.be.revertedWith("Genesis: not owner");
  });
  ...
  it("Should revert if accessories are provided in wrong order", async function () {
    ...
    await expect(
      genesisNFT
        .connect(bob)
        .equipAccessories(1, accessoriesToEquip, { gasLimit: 800000 })
    ).to.be.revertedWith("Genesis: wrong order");
  });
  ...
  it("Should revert if provide more than 4 accessories to equip", async function () {
    ...
    await expect(
      genesisNFT.connect(bob).equipAccessories(1, accessoriesToEquip)
    ).to.be.revertedWith("Genesis: wrong length");
  });
