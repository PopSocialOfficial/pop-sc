...
describe("Cool Cappy NFT testing", function () {
    ...
    it("Should only allow 3 mints per whitelisted", async function() {
        ...
        await expect(genesisNFT.connect(bob).safeMint(bob.address, merklTreeRoot.getHexProof(keccak256(bob.address)))).to.not.be.reverted;
        await expect(genesisNFT.connect(bob).safeMint(bob.address, merklTreeRoot.getHexProof(keccak256(bob.address)))).to.not.be.reverted;
        await expect(genesisNFT.connect(bob).safeMint(bob.address, merklTreeRoot.getHexProof(keccak256(bob.address)))).to.be.revertedWith('CoolCappy: max 3 per wallet');
    });
    ...
    it("Should not be able to claim if not whitelisted", async function() {
        ...
        await genesisNFT.setWhitelistMerkleRoot(merklTreeRoot.getHexRoot());
        await expect(genesisNFT.safeMint(bob.address, merklTreeRoot.getHexProof(keccak256(bob.address)))).to.be.revertedWith('CoolCappy: invalid merkle proof');
    });
    ...
    it("Should revert if not token owner and equip", async function() {
        ...
        await expect(genesisNFT.connect(bob).equipAccessories(1, accessoriesToEquip, {gasLimit: 800000})).to.be.revertedWith("CoolCappy: not owner");
    });
    ...
    it("Should revert if accessories are provided in wrong order", async function() {
        ...
        await expect(genesisNFT.connect(bob).equipAccessories(1, accessoriesToEquip, {gasLimit: 800000})).to.be.revertedWith("CoolCappy: wrong order");
    });
    ...
    it("Should revert if provide more than 4 accessories to equip", async function() {
        ...
        await expect(genesisNFT.connect(bob).equipAccessories(1, accessoriesToEquip)).to.be.revertedWith("CoolCappy: wrong length");
    });
