...
contract CoolCappy is Initializable, ERC721Upgradeable, AccessControlUpgradeable, IERC4906Upgradeable, IERC1155ReceiverUpgradeable {
    ...
    modifier validAccessories(Accessory[] memory _accessories) {
        require(_accessories.length <= accessorySlots, "CoolCappy: wrong length");
        for(uint256 i; i < _accessories.length; i++){
            require(whitelistedContracts[_accessories[i].contractAddr] == true, "CoolCappy: not whitelisted");
            require(accessoryOrder[i] == _accessories[i].contractAddr, "CoolCappy: wrong order");
        }
        _;
    }
    ...
    function deEquipAccessory(uint256 _tokenId, uint256 accessoryType) external isTokenOwner(_tokenId) {
        require(accessoryType <= accessorySlots, 'CoolCappy: invalid accessoryType');
        require(equippedAccessories[_tokenId][accessoryType].accessoryId != 0, "CoolCappy: accessory already de-equipped");
    ...
    function setWhitelisted(address[] memory _whitelistedContracts, uint8 _accessorySlots) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_whitelistedContracts.length == _accessorySlots, "CoolCappy: wrong length");
    ...
    function addWhitelistedContract(address _contractAddr) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_contractAddr != address(0), 'CoolCappy: no address 0');
    ...
    function mintBatch(address[] calldata toAddresses) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(toAddresses.length <= MAX_ADMIN_MINT - adminMintedCount, "CoolCappy: Exceeds admin mint limit");
