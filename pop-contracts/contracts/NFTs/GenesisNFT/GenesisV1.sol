...
contract GenesisV1 is Initializable, ERC721Upgradeable, AccessControlUpgradeable, IERC4906Upgradeable, IERC1155ReceiverUpgradeable {
	...
	function initialize(uint256 _totalSupply, uint256 _startTime, uint256 _salePrice, address _relayer) initializer public {
	...
	function safeMint(address to, bytes32[] calldata merkleProof) external payable {
		...
		if (whitelistMerkleRoot != bytes32(0)) {
			require(MerkleProof.verify(merkleProof, whitelistMerkleRoot, keccak256(abi.encodePacked(_msgSender()))), "Genesis: invalid merkle proof");
		}
