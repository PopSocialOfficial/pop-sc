...
async function main() {
  ...
  console.log(admin);
  let singer_address = "0x42c4e30b6af9C1b730F016C0B29dCc3Ab41bb745";
  ...
  // prod
  // let hat_url = "https://ipfs.popsocial.io/popbit/json/hat/{id}"
  const hatDeploy = await upgrades.deployProxy(
    HatNFT,
    ["Popbit Hat", "HAT", hat_url, 125, singer_address],
  ...
  console.log(
    await upgrades.erc1967.getImplementationAddress(hatDeploy.address),
    "implementation"
  );
  console.log(
    await upgrades.erc1967.getAdminAddress(hatDeploy.address),
    "admin address"
  );
  ...
  const clothesDeploy = await upgrades.deployProxy(
    ClothesNFT,
    ["Popbit Clothes", "CLOTHES", cloth_url, 125, singer_address],
    { initializer: "initialize" }
  );
  sleep(10000);
  ...
  sleep(10000);
  console.log(
    `Clothes Accessory Proxy deployed at address ${clothesDeploy.address}`
  );
  console.log(
    await upgrades.erc1967.getImplementationAddress(clothesDeploy.address),
    "implementation"
  );
  console.log(
    await upgrades.erc1967.getAdminAddress(clothesDeploy.address),
    "admin address"
  );
}
