    ...
    ReverseClaimer
{
    ...
    bytes32 private constant ETH_LABELHASH =
        0x463e999eb3891010d7462a234a75f9ecdc655fa421188ecf1c8f7c556a8533f3;
    ...
    modifier onlyTokenOwnerOrMinter(bytes32 node) {
        bool _canModifyName = canModifyName(node, msg.sender) ||
            (msg.sender == minter && minter != address(0));
        if (!_canModifyName) {
            revert Unauthorised(node, msg.sender);
        }
    ...
    function setSubnodeOwner(
        ...
        uint32 fuses,
        uint64 expiry
    ) public onlyTokenOwnerOrMinter(parentNode) returns (bytes32 node) {
    ...
    function _wrapETH2LD(
        ...
        bytes memory name = _addLabel(label, "\x03pop\x00");
        names[node] = name;
    ...
    /**
     * @notice Set the minter contract. Only the owner can do this
     * @param _minter The new minter contract address
     */
