...
contracts.forEach(function ([ENS, lang]) {
  contract('ENS ' + lang, function (accounts) {
    ...
    it('should allow the creation of subnodes', async () => {
      let result = await ens.setSubnodeOwner('0x0', sha3('pop'), accounts[1], {
        from: accounts[0],
      })

      assert.equal(await ens.owner(namehash.hash('pop')), accounts[1])
