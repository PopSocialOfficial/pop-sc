...
contract('TestRegistrar', function (accounts) {
  ...
  it('registers names', async () => {
    await registrar.register(sha3('pop'), accounts[0], { from: accounts[0] })
  ...
  it('forbids transferring names within the test period', async () => {
    await registrar.register(sha3('pop'), accounts[1], { from: accounts[0] })
    await exceptions.expectFailure(
      registrar.register(sha3('pop'), accounts[0], { from: accounts[0] }),
    )
  })
