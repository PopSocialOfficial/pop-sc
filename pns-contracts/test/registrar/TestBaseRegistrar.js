const ENS = artifacts.require('./registry/PNSRegistry')
...
contract('BaseRegistrar', function (accounts) {
  ...
  before(async () => {
    ...
    registrar = await BaseRegistrar.new(ens.address, namehash.hash('pop'), {
      from: ownerAccount,
    })
    await registrar.addController(controllerAccount, { from: ownerAccount })
    await ens.setSubnodeOwner('0x0', sha3('pop'), registrar.address)
  })
  ...
  it('should allow new registrations', async () => {
    ...
    assert.equal(
      await ens.owner(namehash.hash('newname.pop')),
      registrantAccount,
    )
  ...
  it('should allow registrations without updating the registry', async () => {
    ...
    var block = await web3.eth.getBlock(tx.receipt.blockHash)
    assert.equal(await ens.owner(namehash.hash('silentname.pop')), ZERO_ADDRESS)
  ...
  it('should permit the owner to reclaim a name', async () => {
    await ens.setSubnodeOwner(ZERO_HASH, sha3('pop'), accounts[0])
    await ens.setSubnodeOwner(
      namehash.hash('pop'),
      sha3('newname'),
      ZERO_ADDRESS,
    )
    assert.equal(await ens.owner(namehash.hash('newname.pop')), ZERO_ADDRESS)
    await ens.setSubnodeOwner(ZERO_HASH, sha3('pop'), registrar.address)
  ...
  it('should allow the owner to set a resolver address', async () => {
    ...
    assert.equal(await ens.resolver(namehash.hash('pop')), accounts[1])
  })
})
