...
describe('Name Wrapper', () => {
  ...
  before(async () => {
    ...
    EnsRegistry = await deploy('PNSRegistry')
    ...
    expect(await EnsRegistry.owner(namehash('pop'))).to.equal(
      BaseRegistrar.address,
    )
  })
  ...
  describe('wrap()', () => {
    ...
    it('Does not allow wrapping .eth 2LDs.', async () => {
      ...
      await EnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await expect(
        NameWrapper.wrap(encodeName('wrapped.pop'), account2, EMPTY_ADDRESS),
    ...
    it('Rewrapping a previously wrapped unexpired name retains PCC', async () => {
      ...
      const subWrappedTokenId = namehash(`${subLabel}.${label}.pop`)
  ...
  describe('unwrap()', () => {
    ...
    it('Will not allow previous owner to unwrap name when name expires', async () => {
      ...
      await expect(
        ...
        `Unauthorised("${namehash('sub.unwrapped.pop')}", "${account}")`,
      )
    })
    ...
    it('Will not unwrap .eth 2LDs.', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('pop'), labelhash('unwrapped'), account),
      ).to.be.revertedWith('IncompatibleParent()')
    })
    ...
    it('Will not allow to unwrap with PCC/CU burned if expired', async () => {
      ...
      expect(await EnsRegistry.owner(namehash('sub.awesome.pop'))).to.equal(
        NameWrapper.address,
      )

      await expect(
        NameWrapper.unwrap(namehash('awesome.pop'), labelhash('sub'), account),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.awesome.pop')}", "${account}")`,
      )
    })
    ...
    it('Will not allow to unwrap a name with the CANNOT_UNWRAP fuse burned if not expired', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('abc.pop'), labelhash('sub'), account),
      ).to.be.revertedWith(`OperationProhibited("${namehash('sub.abc.pop')}")`)
    })
  ...
  describe('wrapETH2LD()', () => {
    ...
    const labelHash = labelhash(label)
    const nameHash = namehash(label + '.pop')
    ...
    it('Can re-wrap a name that was wrapped has already expired on the .eth registrar', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('wrapped2.pop'), EMPTY_ADDRESS)
      await expect(tx)
        .to.emit(NameWrapper, 'TransferSingle')
        .withArgs(account2, account, EMPTY_ADDRESS, nameHash, 1)
      await expect(tx)
        .to.emit(NameWrapper, 'NameWrapped')
        .withArgs(
          namehash('wrapped2.pop'),
          encodeName('wrapped2.pop'),
          account2,
    ...
    it('correctly reports fuses for a name that has expired and been rewrapped more permissively', async () => {
      ...
      ;[, fuses] = await NameWrapper.getData(namehash('sub.wrapped2.pop'))
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('wrapped2.pop'))
  ...
  describe('unwrapETH2LD()', () => {
    ...
    it('Allows the owner to unwrap a name.', async () => {
      ...
      expect(await NameWrapper.ownerOf(namehash('unwrapped.pop'))).to.equal(
        account,
      )
      await NameWrapper.unwrapETH2LD(labelHash, account, account)
      // transfers the controller on the .eth registrar to the target address.
      expect(await EnsRegistry.owner(namehash('unwrapped.pop'))).to.equal(
        account,
      )
    ...
    it('emits Unwrap event', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('unwrapped.pop'), account)
    })
    ...
    it('Does not allow a name to be unwrapped if CANNOT_UNWRAP fuse has been burned', async () => {
      ...
      await expect(
        ...
        'OperationProhibited("0x13af6a433a49551e03514f5259bfe18b4fa69c1cc4e627a2b95a06ede7796812")',
      )
    })
    it('Unwrapping a previously wrapped unexpired name retains PCC and expiry', async () => {
      // register and wrap a name with PCC
  ...
  describe('approve()', () => {
    ...
    const subWrappedTokenId = namehash('sub.' + label + '.pop')
    let parentExpiry
    let result
    before(async () => {
      result = await ethers.provider.send('evm_snapshot')
    ...
    it('Does not allow approved accounts to extend expiry when expired', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.subdomain.pop')}")`,
      )
    })
    ...
    it('Approved address cannot unwrap .pop', async () => {
      await NameWrapper.approve(account2, wrappedTokenId)
      await expect(
    ...
    it('Approved address cannot unwrap non .pop', async () => {
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
        'sub',
        account,
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.pop'))
      await expect(
        NameWrapper2.unwrap(wrappedTokenId, labelhash('sub'), account2),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.subdomain.pop')}", "${account2}")`,
      )
    })
    ...
    it('Approval is cleared on unwrap()', async () => {
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.pop'))
      expect(
        await NameWrapper.getApproved(namehash('sub.subdomain.pop')),
      ).to.equal(account2)
  ...
  describe('getApproved()', () => {
    ...
    it('Returns returns zero address when ownerOf() is zero', async () => {
      expect(await NameWrapper.ownerOf(namehash('unminted.pop'))).to.equal(
        EMPTY_ADDRESS,
      )
      expect(await NameWrapper.getApproved(namehash('unminted.pop'))).to.equal(
        EMPTY_ADDRESS,
      )
    })
  ...
  describe('upgrade()', () => {
    describe('.pop', () => {
      const encodedName = encodeName('wrapped2.pop')
      const label = 'wrapped2'
      const labelHash = labelhash(label)
      const nameHash = namehash(label + '.pop')
    ...
    describe('other', () => {
      ...
      const name = label + '.' + parentLabel + '.pop'
      const parentLabelHash = labelhash(parentLabel)
      const parentHash = namehash(parentLabel + '.pop')
  ...
  describe('setFuses()', () => {
    ...
    const wrappedTokenId = namehash('fuses.pop')
    ...
    it('cannot burn fuses as the previous owner of a .eth when the name has expired', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('abc.pop'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('abc.pop')}", "${account}")`,
      )
    })
    ...
    it('cannot burn fuses as a previous owner of a non .eth when the name has expired', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.pop'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.abc.pop')}", "${account}")`,
      )
    })
    ...
    it('Will not allow burning fuses if PARENT_CANNOT_CONTROL has not been burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("0xb8fb3db0ef6f4b21a955f922fc228c86978c4a1411ffca9b2f21b061b33394be")`,
      )
    })
    ...
    it('Will not allow burning fuses of subdomains if CANNOT_UNWRAP has not been burned', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.pop'), CANNOT_TRANSFER),
      ).to.be.revertedWith(
        `OperationProhibited("0xb8fb3db0ef6f4b21a955f922fc228c86978c4a1411ffca9b2f21b061b33394be")`,
      )
    })
    ...
    it('can set fuses and burn transfer', async () => {
      ...
      const wrappedTokenId = namehash('fuses3.pop')
    ...
    it('can set fuses and burn canCreateSubdomains', async () => {
      ...
      expect(
        await EnsRegistry.owner(namehash('creatable.fuses2.pop')),
      ).to.equal(NameWrapper.address)

      expect(
        await NameWrapper.ownerOf(namehash('creatable.fuses2.pop')),
      ).to.equal(account)
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('uncreatable.fuses2.pop')}")`,
      )
    })
  })
  ...
  describe('extendExpiry()', () => {
    ...
    it('Allows parent owner to set expiry without CAN_EXTEND_EXPIRY burned', async () => {
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('sub.fuses.pop'))
    ...
    it('Does not allow owner of .eth 2LD to set expiry', async () => {
      ...
      let [, fuses, expiry] = await NameWrapper.getData(namehash('fuses.pop'))

      expect(fuses).to.equal(IS_DOT_ETH | PARENT_CANNOT_CONTROL | CANNOT_UNWRAP)

      await expect(
        NameWrapper.extendExpiry(namehash('pop'), tokenId, expiry),
      ).to.be.revertedWith(`OperationProhibited("${wrappedTokenId}")`)
    })
    ...
    it('Does not allow extendExpiry() to be called on unwrapped names', async () => {
      ...
      let registryOwner = await EnsRegistry.owner(namehash('sub.fuses.pop'))
    ...
    it('Emits Expiry Extended event', async () => {
      ...
      await expect(tx)
        ...
        .withArgs(namehash(`sub.${label}.pop`), parentExpiry.add(GRACE_PERIOD))
    })
  })
  ...
  describe('setChildFuses()', () => {
    ...
    it('Does not allow parent owners to burn parent controlled fuses after burning PCC', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.fuses.pop')}")`,
      )
    })
    ...
    it('Does not allow non-parent owners to set child fuses', async () => {
      const subWrappedTokenId = namehash('sub.fuses.pop')
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
    ...
    it('should not allow .eth to call setChildFuses()', async () => {
      ...
      ).to.be.revertedWith(`Unauthorised("${namehash('pop')}", "${account}")`)
    })
  ...
  describe('setSubnodeOwner()', async () => {
    ...
    it('Can be called by the owner of a name and sets this contract as owner on the ENS registry.', async () => {
      ...
      expect(await EnsRegistry.owner(namehash(`sub.${label}.pop`))).to.equal(
        NameWrapper.address,
      )

      expect(await NameWrapper.ownerOf(namehash(`sub.${label}.pop`))).to.equal(
        account,
      )
    })
    it('Can be called by an account authorised by the owner.', async () => {
    ...
    it('Fuses cannot be burned if the name does not have PARENT_CANNOT_CONTROL burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`${label2}.${label}.pop`)}")`,
      )
    })
    ...
    it('Does not allow IS_DOT_ETH to be burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`sub.${label}.pop`)}")`,
      )
    })
    ...
    it('Expired subnames should still be protected by CANNOT_CREATE_SUBDOMAIN on the parent', async () => {
      ...
      const subWrappedTokenId = namehash(`${subLabel}.${label}.pop`)
      const subWrappedTokenId2 = namehash(`${subLabel2}.${label}.pop`)
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(labelHash)
      // Confirm that the name is wrapped
      expect(await NameWrapper.ownerOf(wrappedTokenId)).to.equal(account)
  ...
  describe('setSubnodeRecord()', async () => {
    ...
    it('Fuses will remain 0 if expired', async () => {
      ...
      const [, fuses] = await NameWrapper.getData(namehash(`sub.${label}.pop`))
      expect(fuses).to.equal(0)
    })
    ...
    it('Sets the appropriate values on the ENS registry', async () => {
      ...
      const node = namehash(`sub.${label}.pop`)
  ...
  describe('setRecord()', () => {
    ...
    it('Setting the owner to 0 reverts if CANNOT_UNWRAP is burned', async () => {
      ...
      const wrappedTokenId2 = namehash('setrecord2.pop')
      const subWrappedTokenId = namehash('sub.setrecord2.pop')
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId2,
        'sub',
        account,
  ...
  describe('getData', () => {
    ...
    const subLabel = 'sub'
    const subLabelHash = labelhash(subLabel)
    const subNameHash = namehash(`${subLabel}.${label}.pop`)
    const subSubLabel = 'subsub'
    const subSubLabelhash = labelhash(subSubLabel)
    const subSubNameHash = namehash(`${subSubLabel}.${subLabel}.${label}.pop`)
  ...
  describe('registerAndWrapETH2LD()', () => {
    ...
    it('Does not allows fuse to be burned if CANNOT_UNWRAP has not been burned.', async () => {
      ...
      ).to.be.revertedWith(`OperationProhibited("${namehash(label + '.pop')}")`)
    })
  ...
  describe('isWrapped(bytes32 node)', () => {
    ...
    it('identifies an eth name registered on old controller as unwrapped', async () => {
      ...
      const labelHash = labelhash(label)
      const tokenId = namehash('oldcontroller.pop')
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
    ...
    it('identifies an unregistered .eth name as unwrapped', async () => {
      const tokenId = namehash('abcdefghijklmnop.pop')
      expect(await NameWrapper['isWrapped(bytes32)'](tokenId)).to.equal(false)
    })
    ...
    it('identifies a wrapped subname', async () => {
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
      const subTokenId = namehash('sub.something.pop')
  ...
  describe('NameWrapper.names preimage dictionary', () => {
    it('Does not allow manipulating the preimage db by manually setting owner as NameWrapper', async () => {
      ...
      const sub1Domain = sub1Label + '.' + label + '.pop' // sub1.base.eth
  ...
  describe('Grace period tests', () => {
    ...
    const wrappedTokenId = namehash(label + '.pop')
    const subLabel = 'sub'
    const subTokenId = namehash(subLabel + '.' + label + '.pop')
    let parentExpiry
    before(async () => {
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      ;[, , parentExpiry] = await NameWrapper.getData(wrappedTokenId)
    ...
    it('When a .eth name is in grace period, unexpired subdomains can call setSubnodeOwner', async () => {
      ...
      expect(await NameWrapper.ownerOf(namehash('sub2.sub.test.pop'))).to.equal(
        account2,
      )
    })
  ...
  describe('Registrar tests', () => {
    ...
    const wrappedTokenId1 = namehash('sub1.pop')
    ...
    it('Reverts when attempting to call token owner protected function on an unwrapped name', async () => {
      ...
      // registrar to directly register `sub1.pop` to himself, without wrapping
      // the name.
      ...
      //
      // XXX: corrently, `sub1.pop` becomes a normal node
      await EnsRegistryH.setOwner(wrappedTokenId1, NameWrapper.address)

      // create `sub2.sub1.pop` to the victim user with `PARENT_CANNOT_CONTROL`
      // burnt.
      await expect(
  ...
  describe('Implicit Unwrap tests', () => {
    ...
    const wrappedTokenId2 = namehash('sub2.sub1.pop')
    ...
    it('Trying to burn child fuses when re-registering a name on the old controller reverts', async () => {
      ...
      // create `sub2.sub1.pop` w/o fuses burnt
      await NameWrapperH.setSubnodeOwner(
        wrappedTokenId1,
        label2,
        hacker,
      ...
      // XXX: PREPARE HACK!
      // set `EnsRegistry.owner` of `sub1.pop` as the hacker himself.
      ...
      // set `sub2.sub1.pop` to the victim user w fuses burnt
